...
const AdminDashboardPage = () => {
  ...
  const previous =()=>{
    if(viewPage==1){
      globalContext.dispatch({type: "SNACKBAR", payload:{message: "You have reached the Last Page"} })
    }else{
      ...
      <div className="w-full flex justify-center items-center text-gray-700 ">
        <div className="bg-black p-6 px-10 w-10/12 mb-10">
          <div className="flex justify-between mb-16">
            ...
            <button onClick={logout} className="py-3 rounded-full px-6 bg-lime-400 ">
          ...
          {loading?<h2 className="man font-mono text-white text-4xl text-center">Loading...</h2>: videos.map((item, index) => (
            <EachVideo moveVideoItem={moveVideoItem} index={index} key={index} title={item.title} photo={item.photo} user={item.username} like={item.like} id={item.id} />
          ))}

          <div className="mt-6 flex justify-between p-2 mb-4 ">
              <button onClick={previous} className="text-lg font-bold hover:text-lime-400">&larr; Previous</button>
              <button onClick={()=>setViewPage(viewPage+1)} className="text-lg font-bold hover:text-lime-400">  Next &rarr;</button>
          </div>
